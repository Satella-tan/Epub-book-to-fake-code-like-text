    ...
    def emit_fake_filler(fake_ops_count: int, indent: int = 4) -> str:
        ...
        for _i in range(fake_ops_count):
            ...
            if choice == 0:
                ...
                out.append(f"{indent_s}entropy = -sum(p * math.log2(p) for p in empirical_prob_moves if p > 0)")
                out.append(f"{indent_s}Hmax = math.log(3, 2)")
            elif choice == 10:
                out.append(f"{indent_s}r = random.random()")
                out.append(f"{indent_s}bot_move = random.choice(['R', 'P', 'S']) if r < epsilon else bot_move")
            elif choice == 11:
                out.append(f"{indent_s}Z = sum(expEU)")
                out.append(f"{indent_s}softmax_probs = [x / Z for x in expEU]")
            elif choice == 12:
                out.append(f"{indent_s}expected_utility_all_time = [empirical_prob_moves[(j - 1) % 3] for j in range(3)]")
                out.append(f"{indent_s}expected_utility_recent = [empirical_prob_recent[(j - 1) % 3] for j in range(3)]")
            else:
...
def compile_book_to_code_fixed(epub_path: str, out_path: str, seed: int = None, text_ratio: float = 0.5):
    ...
    out_lines.append("import random")
    out_lines.append("")
    ...
    out_lines.append("moves_count_real = [0, 0, 0]")
    out_lines.append("empirical_prob_moves = [0.33, 0.33, 0.34]")
    ...
    out_lines.append("bot_move = 'R'")
    out_lines.append("move_index = 0")
    out_lines.append("x, y, z = (1.0, 2.0, 0.5)")
    out_lines.append("b = 0.5")
    out_lines.append("n = 10")
    out_lines.append("epsilon = 0.2")
    out_lines.append("")
    out_lines.append("class BookEngine:")
    out_lines.append("    \"\"\"Auto-generated book engine. Open in your favorite IDE for fake-code reading experience.\"\"\"")
    out_lines.append("    \"\"\"First chapter may very well be random preliminaries\"\"\"")
    out_lines.append("")
